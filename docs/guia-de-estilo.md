...
10. **Rubrica que briga com a rubrica de baixo** (OS-R8, e a régua saiu de colisões
    medidas). Quando duas rubricas se imprimem na **mesma tela** — a fala base de um nó de
    diálogo e a `alfinetada` que a exposição paga —, a de cima não pode contradizer o
    adereço da de baixo nem gastar de antemão o gesto que ela guarda. **Nunca duas mãos
    ocupadas na mesma tela:** a xícara que arrefece contra a xícara «por servir»; os braços
    cruzados contra o botão «preso entre os dedos até o fim da resposta»; a pilha
    endireitada contra a pilha que «fica por endireitar».

    Três testes, e cada um saiu de um achado:

    - **O mesmo adereço pode voltar — se o segundo estado for CONSEQUÊNCIA do primeiro na
      ordem em que a tela imprime.** A farinha que assenta no antebraço e depois é batida
      de uma mão na outra passa; o papel de luto que só se olha, sob uma alfinetada que diz
      «não torna a tocar» nele, não passa.
    - **Alfinetada que diz «não torna a X» PRESSUPÕE um X anterior.** A rubrica de cima não
      pode negar essa pressuposição nem cumpri-la e gastá-la — o X planta-se num nó que
      toda partida atravessa (a abertura), não no próprio beat.
    - **São DUAS telas, não uma.** E1 e E2 conferem-se à parte, e a rubrica tem de
      sobreviver às duas. Uma que passa contra E1 e falha contra E2 é falha.

    Rubrica que **não** colide fica, e deve ficar — ambiente, olhar, tempo, ou um gesto de
    outra parte do corpo. **Treze dos vinte nós com alfinetada mantêm a sua**, e é por isso
    que o beat 3 tem corpo em **E0**, nível em que a alfinetada não existe: cortar por regra
    o que não colide deixaria a fala nua justamente para quem colheu menos.

    Guarda: **GR8-4** no `qa.mjs` — lê a fala e a alfinetada do mesmo nó, e apanha **só a
    repetição verbatim**. A contradição continua a ser trabalho de leitura.
...
- **A escala absoluta de tempo** (`src/data/seed.js`): morte às 21h de 13/out (hora
  −3); chegada às 13h de 14/out (hora 13).
